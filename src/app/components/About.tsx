        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-16">
          <div className="organics rounded-2xl p-8">
            ...
            <div className="space-y-4 text-[#c8c0b6] leading-relaxed">
              <p>
                {language === 'zh'
                  ? '不是 “AI fortune telling website”。更好的定义是：面向国际用户�?Chinese Metaphysics content + tools + personalized insights platform�?
          ...
          <div className="organics rounded-2xl p-8">
            <h3 className="text-2xl font-serif font-bold text-white mb-6">
              {language === 'zh' ? '必须明确的边�? : 'Boundaries That Must Be Clear'}
            </h3>
            <ul className="space-y-3 text-[#c8c0b6] text-sm leading-relaxed">
              <li><span className="text-[#5ba4a4] mr-2">�?/span>{language === 'zh' ? '不承�?guaranteed prediction / 100% accuracy' : 'No guaranteed prediction or 100% accuracy claims'}</li>
              <li><span className="text-[#5ba4a4] mr-2">�?/span>{language === 'zh' ? '不输出医疗、法律、投资等高风险建�? : 'No medical, legal, or investment advice'}</li>
              <li><span className="text-[#5ba4a4] mr-2">�?/span>{language === 'zh' ? '明确出生数据如何使用、是否存储、能否删�? : 'Clearly explain how birth data is used, stored, and deleted'}</li>
