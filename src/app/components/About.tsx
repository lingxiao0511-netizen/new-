    ...
    <section id="method" className="py-24 relative z-10">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-serif font-bold mb-4" style={{ color: 'var(--text)' }}>
            {language === 'zh' ? 'Clarity, Trust & Boundaries' : 'Clarity, Trust & Boundaries'}
          </h2>
          <div className="w-12 h-0.5 bg-[#5ba4a4] mx-auto opacity-50"></div>
          <p className="text-lg text-light max-w-3xl mx-auto mt-4" style={{ color: 'var(--text-light)' }}>
            {language === 'zh'
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-16">
          <div className="organics rounded-2xl p-8">
            <h3 className="text-xl font-serif font-bold mb-6" style={{ color: 'var(--text)' }}>
              {language === 'zh' ? '这个站应该怎么介绍自己' : 'How the Site Should Present Itself'}
            </h3>
            <div className="space-y-4 text-sm leading-relaxed" style={{ color: 'var(--text-light)' }}>
              <p>
                {language === 'zh'
          ...
          <div className="organics rounded-2xl p-8">
            ...
            <ul className="space-y-3 text-sm leading-relaxed" style={{ color: 'var(--text-light)' }}>
        ...
        <div className="organics rounded-2xl p-8 md:p-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {['About', 'Methodology', 'FAQ', 'Disclaimer', 'Privacy Policy', 'Terms'].map((item) => (
              <div key={item} className="rounded-lg border border-[rgba(160,148,136,0.25)] bg-[rgba(0,0,0,0.05)] px-4 py-3 text-sm" style={{ color: 'var(--text-light)' }}>
                {item}
              </div>
            ))}
          </div>
